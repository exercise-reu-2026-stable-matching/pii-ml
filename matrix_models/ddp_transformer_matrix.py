...
with open(f"saved_transformer_models/{SAVE_MODEL_SUBDIR}/{DATA_NAME}_iter{ITER_INDEX}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}.json", "w") as f:
    json.dump(
        {
...
def plot_data_to_csv(learning_rates, train_losses, test_losses, train_accuracies, test_accuracies, local_epoch, start_epoch):
    ...
    df.to_csv(f"transformer_matrix_plot_data/{DATA_NAME}_iter{ITER_INDEX}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}.csv", index=False)

def save_model(model: DDP, epoch: int, sub_directory: str):
    file_name = f"{DATA_NAME}_iter{ITER_INDEX}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}_ep{epoch:04d}"
...
if RANK == 0:
    ...
    plt.savefig(f"transformer_matrix_plots/{DATA_NAME}_iter{ITER_INDEX}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}_loss")
    ...
    plt.savefig(f"transformer_matrix_plots/{DATA_NAME}_iter{ITER_INDEX}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}_acc")
    ...
    # plt.savefig(f"transformer_matrix_plots/{data_name}_iter{iter_index}_ID{JOB_ID}_n{N}_hs{HIDDEN_SIZE}_bs{BATCH_SIZE}_ws{WORLD_SIZE}_{MASK_OUT_STR}_lr")
