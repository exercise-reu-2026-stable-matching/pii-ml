...
#SBATCH --job-name=pii-ddp
#SBATCH --output=slurm_output/%A.out
#SBATCH --error=slurm_output/%A.err
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=1
#SBATCH --time=24:00:00

echo "[JOB] Running job on ${SLURMD_NODENAME}"
...
ls ${WDIR} | grep -xvF torch_venv | xargs rm -rf --
...
PYTHONUNBUFFERED=1 # Disable buffering to get outputs immediately
export MASTER_ADDR=$(scontrol show hostname ${SLURM_NODELIST} | head -n 1)

echo "[JOB] About to run python script"
srun python3 -m torch.distributed.run --nnodes=${SLURM_NNODES} --nproc_per_node=1 --rdzv_id=${SLURM_JOB_ID} --rdzv_backend=c10d --rdzv_endpoint=$MASTER_ADDR:29400 ${HDIR}/ddp_transformer_matrix.py
# python3 -u ${HDIR}/transformer_matrix.py ${CONFIG_DIR}/${CONFIG_FILE}
